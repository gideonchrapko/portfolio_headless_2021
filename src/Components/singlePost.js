...
const SinglePost = () => {
  ...
  function leftModalImage() {
    const modalImgLength =
      singlePost[0].sectionContent[modalImage].sectionImages.length
    if (modalIndex === 1) {
  ...
  return (
    <Container>
      <NavHead />
      <NavBar slugRoute={slugRoute} postData={postData} />
      {previewModal ? (
          ...
          2 ? (
            <div
              ...
              style={{
                backgroundImage: `url(${urlFor(
                  singlePost[0].sectionContent[modalImage].sectionImages[1]
                    .asset
                ).url()})`,
              }}
            ></div>
            ...
            <>
              <div
                ...
                style={{
                  backgroundImage: `url(${urlFor(
                    singlePost[0].sectionContent[modalImage].sectionImages[
                      modalIndex
                    ].asset
                  ).url()})`,
                }}
              ...
              <img
                ...
                onClick={() => rightModalImage()}
              />
              <img
                alt="View Previous"
                src={Arrow}
                className="modal-arrow-left"
                onClick={() => leftModalImage()}
              />
              <div
                style={{
                  width: '100vw',
          ...
          singlePost.map((project, index) => (
            <span key={index}>
              <Row>
                <Col
                  lg={{ span: 1 }}
                  style={{ position: 'fixed', left: '0', width: '12%' }}
                  className="d-xs-none d-md-none d-none d-lg-block"
                >
                  {project &&
                    !tldr &&
                    project.sectionTitle.map((index, i) => {
                      const indexShorthand =
                        index.length > 10
                          ? index.substr(0, 20 - 1) + '...'
                          : index
                      return (
                ...
                <Col lg={{ span: 10, offset: 1 }}>
                  ...
                  >
                    <h1 className="tldr-project-title">
                      {tldr ? project.projectTitle : ''}
                    </h1>
                    <div
                      className="sing-proj-modular-tldr"
                      style={{ width: '100%', textAlign: 'right' }}
                    >
                      {project.tldrLink ? (
                        <h6>
                          <a href={project.tldrLink} style={{ color: 'black' }}>
                            {tldr ? 'View Site' : ''}
                          </a>
                        </h6>
                      ) : null}
                  ...
                  >
                    <h1 className="sing_proj_headerTitle">
                      {!tldr ? project.projectTitle : ''}
                    </h1>
                    {!tldr ? (
                      <img
                        src={project.thumbImage.asset.url}
                        alt={`${project.projectTitle} logo`}
                        className="sing-proj-thumb"
                      />
                    ) : (
                      <span></span>
                    )}
                  ...
                  {tldr ? (
                    <div
                      style={{
                        backgroundImage: `url(${project.tldrClip.asset.url})`,
                      }}
                ...
                >
                  {mobileView ? (
                    <div
                      ...
                      style={{
                        background: `${tldr ? 'transparent' : '#00E4B6'}`,
                        border: `${tldr ? '2px solid #00E4B6' : ''}`,
                      }}
                    ...
                    >
                      <h1
                        style={{ color: `${tldr ? '#00E4B6' : 'white'}` }}
                        className="tldr-font-mobile"
                      >
                        TLDR
                      </h1>
                    </div>
                    ...
                    <div
                      style={{
                        background: `${tldr ? 'white' : '#00E4B6'}`,
                        border: `${tldr ? '5px solid #00E4B6' : ''}`,
                        transform: `translateX(${hover ? '20px' : '30px'})`,
                      }}
                      onClick={() => setTldr(!tldr)}
                      onPointerOver={() => setHover(true)}
                      onPointerOut={() => setHover(false)}
                ...
                <>
                  ...
                  <Row className="gx-3">
                    <Col lg={{ span: 2, offset: 1 }} xs={6}>
                      ...
                      <div
                        className="sing-proj-modular-cont"
                        style={{ paddingBottom: '5vw', paddingLeft: '15px' }}
                      >
                        {project.role &&
